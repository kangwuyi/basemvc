...
另有[UglifyJS3中文文档在此](https://segmentfault.com/a/1190000010874406)
...
- `uglify-js`的发行版本只支持ES5，如果你要压缩ES6+代码请使用[兼容](https://segmentfault.com/a/1190000008995453#harmony)开发分支
...
- `keep_fnames` -- 默认 `false`。传 `true`来防止压缩器干掉函数名。对那些依赖`Function.prototype.name`的函数很有用。延展阅读：`keep_fnames` [混淆选项](https://segmentfault.com/a/1190000008995453#mangle).
...
- `inline-script` (默认 `false`) -- 避免字符串中出现`中的斜杠
...
例如[Acorn](https://github.com/ternjs/acorn) ，这是一个超级快的生成SpiderMonkey AST的解释器。它带有一个实用的迷你CLI，能解释一个文件、把AST转存为JSON并标准输出。可以这样用UglifyJS来压缩混淆：
...
sourcemap（查阅[规格](https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit)）中的`file`属性会优先使用 `outFileName` ，假如没有，会从`outSourceMap`中推导（就是去掉`'.map'`）。
...
- `output` (默认 `null`) — 如果你要进一步指定[输出选项](http://lisperator.net/uglifyjs/codegen),请传一个对象。默认是压缩到最优化。
- `compress` (默认 `{}`) — 传`false`的话就跳过整个压缩步骤。自定义的话请传一个[压缩选项](http://lisperator.net/uglifyjs/compress)对象。
- `parse` (默认 {}) — 如果你要进一步自定义解释步骤请传一个[解释选项](http://lisperator.net/uglifyjs/parser)对象（不是所有选项都有效....下面再说）。
...
- `except` - 传一个应该排除在混淆之外的标识的数组。
  ...
  - `keep_fnames` -- 默认`false`。传`true`的话就不混淆函数名。对那些依赖`Function.prototype.name`的代码有用。延展阅读：`keep_fnames` [压缩选项](https://segmentfault.com/a/1190000008995453#compressor-options).
...
你需要在调用`print`时传`source_map`参数。`source_map`参数需要是`SourceMap`对象（在[source-map](https://github.com/mozilla/source-map)库顶部有个小框框里说了）。
...
- `root`: `sourceRoot` 属性 (详看 [规格](https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit))
